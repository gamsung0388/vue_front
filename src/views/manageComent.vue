<template>
    <div class="commentList">
        <div v-if="selected==false">
            ...
            <router-link to="/manage"><button>게시글 전환</button></router-link>
            <table border="1">
                <thead>
                <tr>
                ...
                <tr v-for="(vitem, idx) in list" :key="idx">
                    ...
                    <input 
                        ...
                        :id="vitem.commentNum"
                        name="chk"
                        :value="vitem.commentNum"
                        v-model="selectList" 
                        :key="idx">
                    </th>
                    <td align="center"> {{ vitem.commentNum }}</td>
                    ...
                    <td align="center">{{vitem.commentTxt}}</td>   
                    <td>{{vitem.userId}}</td>                           
                    <td>{{vitem.commentReg}}</td>
                    <td>{{vitem.commentUdt}}</td>
                </tr>
                </tbody>
            </table>
            <div>
                ...
                <button @click="() => commentTwoTrueDelete()">삭제</button>
                <br>
    ...
    methods: {
        ...
        commentSelect(){
          ...
          .then(res => {
            ...
            //게시판 for
            res.data.commentList.forEach(i => {
                //console.log(i.commentNum)
                this.checkList.push(i.commentNum) 
            });
        ...
        commentTwoTrueDelete(){
          ...
          if(this.selectList.length==0){
            ...
            this.commentTrueDelete(params)
          }
        },

        commentTrueDelete(params){
            console.log(params)

          this.axios.get("/comment/truedelete?checkList="+encodeURIComponent(JSON.stringify(params.checkList)))
          .then(res => {
            console.log(res.data);  
            this.commentSelect();
